...
import { LayoutDashboard, Globe, Shield, MessageSquare, Terminal, Menu, X, ShieldOff } from 'lucide-react';
...
import IftttDefense from './components/IftttDefense';
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case ViewState.IFTTT_DEFENSE: return <IftttDefense />;
        ...
        <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 border-r border-slate-800 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex flex-col h-full p-6">
            ...
            <nav className="space-y-2 flex-1">
              ...
              <div className="pt-4 pb-2">
                <p className="px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Shadow IT Defense</p>
              </div>
              <NavItem view={ViewState.IFTTT_DEFENSE} icon={<ShieldOff size={20} />} label="IFTTT Defense" />
            </nav>
            ...
            <header className="flex justify-between items-center mb-8">
              <div>
                <h2 className="text-2xl font-bold text-white">
                  ...
                  {currentView === ViewState.IFTTT_DEFENSE && 'IFTTT / Shadow IT Defense'}
                </h2>
                <p className="text-slate-400 text-sm">
                  ...
                  {currentView === ViewState.IFTTT_DEFENSE && 'OAuth audit, DLP, email forwarding & firewall controls.'}
                </p>
              </div>
